...
public class App extends Application {
    ...
    public Pane showLeaderboard() throws IOException {
        ...
        table.setMaxHeight((board.getHeight() - 3) * board.getCellSize());
        table.setMaxWidth(board.getWidth() * board.getCellSize());

        TableColumn<Player, String> name = new TableColumn<>("Name");
        name.setMinWidth(board.getWidth() * board.getCellSize() / 2.0);
        name.setCellValueFactory(new PropertyValueFactory<>("name"));

        TableColumn<Player, Integer> score = new TableColumn<>("Score");
        score.setMinWidth(board.getWidth() * board.getCellSize() / 2.0);
    ...
    public void startGame() throws IOException{
        ...
        Canvas c = new Canvas(board.getWidth() * board.getCellSize(), board.getHeight() * board.getCellSize());
        GraphicsContext gc = c.getGraphicsContext2D();
        root.getChildren().add(c);

        // Gameplay animation depends on snake's speed as board Rate
        AnimationTimer aTimer = new AnimationTimer(){
            ...
            public void handle(long now) {
                ...
                if (lastCell == 0) {
                    lastCell = now;
                    draw.drawAllComponents(gc, engine, board, playerGame, snake, fruit);
                }
        ...
        Scene gameScene = new Scene(root, board.getWidth() * board.getCellSize(), board.getHeight() * board.getCellSize());

        keyboardListener.listenKey(gameScene, snake);

        // snake's length (5) initialization
        for(int i=0; i<5; ++i){
            snake.bodyCells.add(new Cell(board.getWidth() / 2, board.getHeight() / 2));
        }
